...
describe('ButtonArea', () => {
    ...
    const getProviderButtonHtml = (html: string) => {
        return html.match(/<button[^>]*title="chat\.providerLabel"[\s\S]*?<\/button>/)?.[0] ?? '';
    };

    const getModelButtonHtml = (html: string) => {
        return html.match(/<button[^>]*title="chat\.modelLabel"[\s\S]*?<\/button>/)?.[0] ?? '';
    };
    ...
    it('disables provider, mode, model, and reasoning selectors while a turn is loading', () => {
        ...
        expect(html).toMatch(/<button[^>]*title="chat\.providerLabel"[^>]*disabled/);
        expect(html).toMatch(/<button[^>]*title="chat\.modeLabel"[^>]*disabled/);
        expect(html).toMatch(/<button[^>]*title="chat\.modelLabel"[^>]*disabled/);
        expect(html).toMatch(/<button[^>]*title="chat\.reasoningLabel"[^>]*disabled/);
    });
    ...
    it('shows a disabled send affordance instead of stop while a send is waiting for request id', () => {
        ...
        expect(html).toMatch(/<button(?=[^>]*title="chat\.send")(?=[^>]*disabled)[^>]*>/);
        expect(html).not.toContain('title="chat.stop"');
    });
    ...
    it('keeps composer primary actions in a fixed right-side action group', () => {
        ...
        expect(sendHtml).toMatch(/<button(?=[^>]*title="chat\.enhancePrompt")(?=[^>]*aria-label="chat\.enhancePrompt")(?=[^>]*chat-composer-action-button)(?=[^>]*h-7)(?=[^>]*w-7)(?=[^>]*shrink-0)[^>]*>/);
        expect(sendHtml).toMatch(/<button(?=[^>]*title="chat\.send")(?=[^>]*aria-label="chat\.send")(?=[^>]*chat-composer-primary-action)(?=[^>]*h-7)(?=[^>]*w-7)(?=[^>]*shrink-0)[^>]*>/);
        expect(stopHtml).toMatch(/<button(?=[^>]*title="chat\.stop")(?=[^>]*aria-label="chat\.stop")(?=[^>]*chat-composer-primary-action)(?=[^>]*h-7)(?=[^>]*w-7)(?=[^>]*shrink-0)[^>]*>/);
    });
    ...
    it('renders an icon-only model refresh control when dynamic refresh is available', () => {
        ...
        expect(html).toMatch(/<button(?=[^>]*title="chat\.modelsRefresh")(?=[^>]*aria-label="chat\.modelsRefresh")[^>]*>/);
    ...
    it('surfaces model refresh loading and error states in the model menu footer', () => {
        ...
        expect(loadingHtml).toContain('chat.modelsRefreshing');
