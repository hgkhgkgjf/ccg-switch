...
## Scenario: Chat Session Terminal Actions

### 1. Scope / Trigger

- Trigger: the Chat session sidebar exposes context-menu actions to open a
  session's project directory in a system terminal or resume the native CLI
  session in a new terminal window.
- This is a frontend-to-backend command boundary with external side effects
  (spawning a terminal process). The frontend owns which session action was
  clicked; backend owns validation and platform terminal launch.

### 2. Signatures

```ts
await invoke<void>('chat_open_project_in_terminal', {
    projectDir: session.projectDir,
});

await invoke<void>('chat_resume_session_in_terminal', {
...
- `SessionMeta.resumeCommand` is generated by the provider session scanner and
  is the source of truth for CLI resume syntax. The Chat UI must not rebuild the
  resume command from `providerId + sessionId`.
- Known scanner outputs are `claude --resume <sessionId>`,
  `codex resume <threadId>`, and `gemini --resume <sessionId>`.
- `chat_open_project_in_terminal` requires a trimmed, non-empty existing
  directory with no control characters. Missing or non-directory paths are
  rejected before spawning a terminal.
- `chat_resume_session_in_terminal` requires a trimmed, non-empty
  `resumeCommand` with no control characters. If `projectDir` is present and is
  an existing directory, backend runs the terminal from that directory; if it is
  absent or invalid, backend runs the resume command without changing the start
  directory.
- Windows terminal launch must pass an empty `start` title and the working
  directory through `start /D <projectDir>`:
  `cmd /c start "" /D <projectDir> cmd /K [resumeCommand]`. Do not concatenate
  `cd /d "<projectDir>"` into the `/K` command string; nested `cmd start` /
  `/K` quote parsing can make `cmd.exe` treat the path as malformed and produce
  `文件名、目录名或卷标语法不正确`.
- macOS / Linux may keep using a shell command that prepends `cd "<projectDir>"`
  before `resumeCommand`, because those branches do not go through Windows
  `cmd.exe start` title parsing.
- The independent `/workspaces` page is not a terminal-action owner. Recent
  sessions, project browsing, and terminal session actions belong in
  `ChatSessionSidebar`; deleting the route must not remove Chat workspace/cwd
  commands such as `chat_workspace_status` or `chat_list_workspace_files`.

### 4. Validation & Error Matrix

| Condition | Required behavior |
|-----------|-------------------|
| `projectDir` is blank for open-in-terminal | Return `Err("工作目录路径为空")`; do not spawn a terminal. |
| `projectDir` contains control characters | Return `Err("工作目录路径包含非法字符")`; do not spawn a terminal. |
| `projectDir` does not exist or is not a directory | Return a directory validation error; do not spawn a terminal. |
| `resumeCommand` is blank | Return `Err("恢复命令为空")`; do not spawn a terminal. |
| `resumeCommand` contains control characters | Return `Err("恢复命令包含非法字符")`; do not spawn a terminal. |
| `resumeCommand` valid and `projectDir` missing/invalid | Spawn terminal with only the resume command. |
| Terminal executable cannot be spawned | Return `Err("无法打开终端: ...")` or the platform-specific terminal error. |

### 5. Good/Base/Bad Cases

- Good: a Codex session row contains `resumeCommand = "codex resume abc"`;
  right-click → `Resume Session in Terminal` invokes
  `chat_resume_session_in_terminal({ resumeCommand: "codex resume abc",
  projectDir })`, and backend opens a terminal in the project directory.
- Base: a session has a valid `projectDir` but no `resumeCommand`; the sidebar
  shows a Toast such as `No resume command available` and does not call backend.
- Bad: frontend sends `{ provider: "codex", sessionId: "abc" }` and Rust maps
  it to `codex --session abc`; this duplicates provider-specific CLI syntax and
  can drift from scanner output.

### 6. Tests Required

- Frontend component test: `ChatSessionSidebar` context menu includes
  open-terminal and resume-terminal actions.
- Frontend component test: resume-terminal invokes
  `chat_resume_session_in_terminal` with `resumeCommand` and `projectDir`, and
  not the legacy `provider/sessionId` shape.
- Frontend navigation test: `/workspaces` is absent from main/top navigation
  after the independent Workspaces page is removed.
- Rust unit test: Windows terminal argument construction includes the empty
  `start` title, uses `/D <projectDir>` for the start directory, and does not
  embed `cd /d` in the `/K` command string.
- Build check: `npm run build` must pass so removed route imports and locale
  cleanup do not leave unused symbols.
- Backend check: `cargo check --manifest-path src-tauri/Cargo.toml` must pass
  so command signatures stay registered in `lib.rs`.
...
```ts
await invoke('chat_resume_session_in_terminal', {
...
Command::new("cmd")
    .args([
        ...
        "cmd",
        "/K",
        &format!("cd /d \"{project_dir}\" && {resume_command}"),
    ])
    .spawn()?;
```
...
```rust
Command::new("cmd")
    .args(["/c", "start", "", "/D", project_dir, "cmd", "/K", resume_command])
    .spawn()?;
```
